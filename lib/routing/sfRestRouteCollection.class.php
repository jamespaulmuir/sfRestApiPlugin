...
class sfRestRouteCollection extends sfObjectRouteCollection
{
  protected
  ...
  protected function generateRoutes()
  {
    ...
    if ($this->options['with_wildcard_routes'])
    {
      ...
      $this->routes[$this->getRoute('object')] = new $this->routeClass(
        ...
        array('module' => $this->options['module'], 'sf_format' => 'json'),
...
protected function getRouteForMeta()
  {
    return new $this->routeClass(
      sprintf('%s/:%s', $this->options['prefix_path'], $this->options['column']),
      array('module' => $this->options['module'], 'action' => $this->getActionMethod('meta'), 'sf_format' => 'json'),
      array_merge($this->options['requirements'], array('sf_method' => 'head')),
      array('model' => $this->options['model'], 'type' => 'object', 'method' => $this->options['model_methods']['object'])
    );
  }
  ...
  {
    $actions = array('list', 'show', 'create', 'update', 'delete', 'meta');
